...
\restrict asihE83fyU9uisfM3k0r4n2zRelJQosFkBdacpHWBZurO5Lnjq8kt4IyHNga1yf
...
CREATE TABLE public.bookings (
    ...
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT bookings_payment_status_check CHECK (((payment_status)::text = ANY ((ARRAY['pending'::character varying, 'paid'::character varying, 'partial'::character varying])::text[]))),
...
CREATE TABLE public.carts (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT carts_delivery_method_check CHECK (((delivery_method)::text = ANY ((ARRAY['pickup'::character varying, 'delivery'::character varying])::text[])))
);
...
CREATE TABLE public.checkout_sessions (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT checkout_sessions_delivery_method_check CHECK (((delivery_method)::text = ANY ((ARRAY['delivery'::character varying, 'pickup'::character varying])::text[]))),
    CONSTRAINT checkout_sessions_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'paid'::character varying, 'cancelled'::character varying, 'failed'::character varying])::text[])))
);
...
CREATE TABLE public.communication_campaigns (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT communication_campaigns_status_check CHECK (((status)::text = ANY ((ARRAY['draft'::character varying, 'scheduled'::character varying, 'sent'::character varying, 'failed'::character varying])::text[]))),
    CONSTRAINT communication_campaigns_type_check CHECK (((type)::text = ANY ((ARRAY['email'::character varying, 'sms'::character varying])::text[])))
);
...
CREATE TABLE public.customer_activity (
    ...
    metadata jsonb,
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT customer_activity_type_check CHECK (((type)::text = ANY ((ARRAY['purchase'::character varying, 'login'::character varying, 'email_open'::character varying, 'email_click'::character varying, 'sms_sent'::character varying, 'loyalty_earned'::character varying, 'loyalty_redeemed'::character varying])::text[])))
);
...
CREATE TABLE public.customer_loyalty (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT customer_loyalty_loyalty_tier_check CHECK (((loyalty_tier)::text = ANY ((ARRAY['bronze'::character varying, 'silver'::character varying, 'gold'::character varying, 'platinum'::character varying])::text[])))
);
...
CREATE TABLE public.loyalty_programs (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT loyalty_programs_type_check CHECK (((type)::text = ANY ((ARRAY['quarterly'::character varying, 'annual'::character varying, 'custom'::character varying])::text[])))
);
...
CREATE TABLE public.orders (
    ...
    CONSTRAINT orders_payment_method_check CHECK (((payment_method)::text = ANY ((ARRAY['online'::character varying, 'on_delivery'::character varying, 'on_pickup'::character varying])::text[]))),
    CONSTRAINT orders_payment_status_check CHECK (((payment_status)::text = ANY ((ARRAY['pending'::character varying, 'partial'::character varying, 'paid'::character varying, 'failed'::character varying, 'refunded'::character varying, 'cancelled'::character varying])::text[]))),
    CONSTRAINT orders_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'confirmed'::character varying, 'processing'::character varying, 'ready_for_pickup'::character varying, 'shipped'::character varying, 'out_for_delivery'::character varying, 'delivered'::character varying, 'completed'::character varying, 'cancelled'::character varying, 'refunded'::character varying])::text[]))),
...
CREATE TABLE public.payments (
    ...
    CONSTRAINT payments_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'partial'::character varying, 'completed'::character varying, 'failed'::character varying, 'refunded'::character varying, 'cancelled'::character varying])::text[])))
);
...
CREATE TABLE public.purchase_history (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT purchase_history_status_check CHECK (((status)::text = ANY ((ARRAY['completed'::character varying, 'pending'::character varying, 'cancelled'::character varying, 'refunded'::character varying])::text[])))
);
...
\unrestrict asihE83fyU9uisfM3k0r4n2zRelJQosFkBdacpHWBZurO5Lnjq8kt4IyHNga1yf
